...
export function handleTool(name, args) {
  ...
  switch (name) {
    case 'list_books': {
      const db = getDb(true);
      const books = db.prepare("SELECT b.id, b.title, b.total_paragraphs, b.created_at, b.cover_image, p.page as current_paragraph_idx, COALESCE(p.paragraph_offset, 0) as current_paragraph_offset FROM books b LEFT JOIN book_progress p ON b.id = p.book_id AND p.reader = 'default' ORDER BY b.created_at DESC").all();
    ...
    case 'update_progress': {
      ...
      db.prepare("INSERT INTO book_progress (book_id, reader, page, paragraph_offset, updated_at) VALUES (?, 'default', ?, ?, datetime('now')) ON CONFLICT(book_id, reader) DO UPDATE SET page = excluded.page, paragraph_offset = excluded.paragraph_offset, updated_at = datetime('now')").run(args.book_id, position.paragraph_idx, position.offset);
      db.close();
      return {
        ok: true,
        page: displayPage,
