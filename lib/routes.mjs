...
import { getDb, getDbPath, getImageDir } from './db.mjs';
...
import { handleCoreadBackupRequest } from './coread-backup.mjs';
...
// 锁文件默认放 coread 自己的 data 目录（开源独立部署不依赖外部路径）；
// 需要与 SullyOS bridge 共享同一把锁时用 COREAD_ROOM_LOCK 指过去。开门要 owner key。
const ROOM_OWNER_KEY = process.env.ROOM_OWNER_KEY || 'xk-room-owner-f47ac10b58d2e619a3c4';
// 锁文件路径惰性求值：initDb 之后才能拿到真实 db 目录（测试里 db 在临时目录）
function roomLockPath() {
  return process.env.COREAD_ROOM_LOCK || path.join(path.dirname(getDbPath() || path.join(process.cwd(), 'data', 'coread.db')), 'reading-room-lock.json');
}
function readRoomLock() {
  try {
    const s = JSON.parse(fs.readFileSync(roomLockPath(), 'utf8'));
    return { locked: !!s.locked, at: s.at || null, note: typeof s.note === 'string' ? s.note : '' };
  } catch { return { locked: false, at: null, note: '' }; }
}
function writeRoomLock(locked, note = '') {
  const state = { locked: !!locked, at: new Date().toISOString(), note: String(note || '') };
  fs.mkdirSync(path.dirname(roomLockPath()), { recursive: true });
  fs.writeFileSync(roomLockPath(), JSON.stringify(state));
  return state;
}
...
export async function handleRequest(req, res, opts = {}) {
  // 共读室备份与恢复增强版（/v1/coread-backup/*）：模块内自己做 owner-key 鉴权，不过门锁
  if (req.url?.startsWith('/v1/coread-backup/')) {
    return handleCoreadBackupRequest(req, res, { json, readRawBody, ownerKey: ROOM_OWNER_KEY });
  }
  // 共读室关门锁（SullyOS 回填）：查锁任何人可以；开门必须 owner key
  if (req.method === 'GET' && req.url === '/v1/reading-room/door') {
    cors(res);
    json(res, 200, readRoomLock());
    return true;
  }
  if (req.method === 'POST' && req.url === '/v1/reading-room/door') {
    cors(res);
    try {
      const body = await readBody(req);
      const locked = !!body.locked;
      if (!locked && (req.headers['x-owner-key'] || '') !== ROOM_OWNER_KEY) {
        json(res, 403, { error: 'owner_key_required', message: '只有主人能开门。' });
        return true;
      }
      const state = writeRoomLock(locked, body.note);
  ...
  if (req.method === 'GET' && (req.url === '/v1/reading-stats' || req.url.startsWith('/v1/reading-stats?'))) {
    ...
    try {
      ...
      // 逐 session 明细（SullyOS 回填：12 周放射图的数据源，start_min=几点开始读，elapsed_ms=读了多久）
      const sessions = db.prepare(`SELECT c.session_id, c.reading_date, c.book_id, COALESCE(b.title, '') AS book_title, c.start_min, c.elapsed_ms
        FROM reading_checkpoints c LEFT JOIN books b ON b.id = c.book_id ORDER BY c.reading_date, c.start_min`).all();
      ...
      json(res, 200, { today, today_seconds: todaySeconds, total_seconds: totalSeconds, ...streaks, daily, books, notes, by_day: byDay, sessions });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  }
  ...
  // POST /v1/books/:id/finish — preserve the first date a book was finished（SullyOS 回填：同时写 reading_status/first/last_finished_at，按 reader 隔离）
  if (req.method === 'POST' && req.url.match(/^\/v1\/books\/\d+\/finish$/)) {
    if (roomGate(req, res)) return true;
    try {
      const id = parseInt(req.url.split('/')[3]);
      // 兼容 SullyOS UI 翻到末页的空 body 自动标读完：缺省用今天
      const body = await readBody(req).catch(() => ({}));
      const finishedDate = String(body.finished_date || new Date().toISOString().slice(0, 10));
      if (!isValidDate(finishedDate)) { json(res, 400, { error: 'valid finished_date required' }); return true; }
      const urlObj = new URL(req.url, `http://localhost:${port}`);
      const reader = urlObj.searchParams.get('reader') || 'default';
      const db = getDb();
      const book = db.prepare('SELECT id, title FROM books WHERE id = ?').get(id);
      if (!book) {
        db.close(); json(res, 404, { error: 'book not found' }); return true;
      ...
      db.prepare(`
        INSERT INTO book_progress (book_id, reader, finished_at, reading_status, first_finished_at, last_finished_at)
        VALUES (?, ?, ?, 'finished', ?, ?)
        ON CONFLICT(book_id, reader) DO UPDATE SET
          finished_at = COALESCE(book_progress.finished_at, excluded.finished_at),
          reading_status = 'finished',
          first_finished_at = COALESCE(book_progress.first_finished_at, excluded.first_finished_at),
          last_finished_at = excluded.last_finished_at
      `).run(id, reader, finishedDate, finishedDate, finishedDate);
      db.prepare(`
      ...
      db.close();
      json(res, 200, { ok: true, finished_at: finishedAt, reading_status: 'finished' });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  }

  // POST /v1/books/:id/reread — 重读：状态翻回 reading，保留读完日期（SullyOS 回填）
  if (req.method === 'POST' && req.url.match(/^\/v1\/books\/\d+\/reread$/)) {
    if (roomGate(req, res)) return true;
    try {
      const id = parseInt(req.url.split('/')[3]);
      const urlObj = new URL(req.url, `http://localhost:${port}`);
      ...
      db.prepare("UPDATE book_progress SET reading_status = 'reading' WHERE book_id = ? AND reader = ?").run(id, reader);
      db.close();
      json(res, 200, { ok: true, id, reader, reading_status: 'reading' });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  }

// from_who 归一（沉哥复审 bug②）：AI 名归 'ai'，空/人类名归 'human'，显示层再映射当前配置名
function normalizeWho(db, w) {
  const s = String(w || '').trim();
  if (!s) return 'human';
  const ls = s.toLowerCase();
  let aiName = '沉';
  try { aiName = db.prepare("SELECT value FROM config WHERE key = 'ai_name'").get()?.value || aiName; } catch {}
  if (ls === 'ai' || ls === 'seth' || s === '沉' || s === aiName) return 'ai';
  return 'human';
}

// GET/POST /v1/settings — 名字设置等跨设备配置（沉哥复审 bug③：localStorage 不跨设备，搬服务端 config 表）
if (req.url === '/v1/settings') {
  if (req.method === 'GET') {
    const db = getDb(true);
    const read = (k, d) => { try { return db.prepare('SELECT value FROM config WHERE key = ?').get(k)?.value ?? d; } catch { return d; } };
    const out = { human_name: read('human_name', '彤宝'), ai_name: read('ai_name', '沉') };
    db.close();
    json(res, 200, out);
    return true;
  }
  if (req.method === 'POST') {
    try {
      const body = await readBody(req);
      const dbw = getDb();
      const w = dbw.prepare('INSERT OR REPLACE INTO config (key, value) VALUES (?, ?)');
      if (typeof body.human_name === 'string' && body.human_name.trim()) w.run('human_name', body.human_name.trim().slice(0, 30));
      if (typeof body.ai_name === 'string' && body.ai_name.trim()) w.run('ai_name', body.ai_name.trim().slice(0, 30));
      dbw.close();
      json(res, 200, { ok: true });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  ...
  // GET /v1/books（SullyOS 回填：多读者 reader 参数 + reading_status/finished/comment_count 字段）
  if (req.method === 'GET' && (req.url === '/v1/books' || req.url.startsWith('/v1/books?'))) {
    try {
      ...
      const books = db.prepare(`SELECT b.id, b.title, b.total_paragraphs, b.created_at, b.cover_image,
        p.page as current_paragraph_idx, COALESCE(p.paragraph_offset, 0) as current_paragraph_offset,
        p.updated_at as last_read_at, p.last_opened_at as last_opened_at,
        p.reading_status, p.first_finished_at, p.last_finished_at
        FROM books b LEFT JOIN book_progress p ON b.id = p.book_id AND p.reader = ?
        ORDER BY MAX(COALESCE(p.last_opened_at, ''), b.created_at) DESC, b.id DESC`).all(reader);
      ...
      const result = books.map(b => {
        ...
        return {
          ...b,
          // status 是给书架徽标用的派生值：finished=读完；其余不贴标
          status: b.reading_status || null,
          current_page: b.current_paragraph_idx,
  ...
  // GET /v1/books/:id/comments — 全量批注（SullyOS UI 开书时单独拉批注，不重算分页）
  ...
  if (req.method === 'GET' && req.url.match(/^\/v1\/books\/\d+(\?|$)/)) {
    ...
    try {
      ...
      const comments = db.prepare('SELECT * FROM book_comments WHERE book_id = ? ORDER BY paragraph_idx, created_at').all(id);
      const progress = db.prepare('SELECT page, COALESCE(paragraph_offset, 0) AS paragraph_offset FROM book_progress WHERE book_id = ? AND reader = ?').get(id, url.searchParams.get('reader') || 'default');
  ...
  if (req.method === 'POST' && req.url.match(/^\/v1\/books\/\d+\/comment$/)) {
    ...
    try {
      ...
      const author = normalizeWho(db, from_who);
  ...
  if (req.method === 'GET' && req.url.match(/^\/v1\/books\/\d+\/reader-state/)) {
    ...
    try {
      ...
      const db = getDb(true);
      const progress = db.prepare('SELECT page FROM book_progress WHERE book_id = ? AND reader = ?').get(id, urlObj.searchParams.get('reader') || 'default');
  ...
  // PATCH /v1/books/:id/progress（多读者：?reader= 隔离，默认 default）
  if (req.method === 'PATCH' && req.url.match(/^\/v1\/books\/\d+\/progress$/)) {
    ...
    try {
      ...
      db.prepare("INSERT INTO book_progress (book_id, reader, page, paragraph_offset, updated_at) VALUES (?, ?, ?, ?, datetime('now')) ON CONFLICT(book_id, reader) DO UPDATE SET page = excluded.page, paragraph_offset = excluded.paragraph_offset, updated_at = datetime('now')").run(id, reader, paragraphIdx, paragraphOffset);
      const pagination = resolvePagination(db, id);
  ...
  // POST /v1/books/:id/open — 只写last_opened_at，不动page/updated_at，没读过的书不伪造进度（多读者：?reader=）
  if (req.method === 'POST' && req.url.match(/^\/v1\/books\/\d+\/open$/)) {
    ...
    try {
      ...
      const reader = urlObj.searchParams.get('reader') || 'default';
      const db = getDb();
      db.prepare("INSERT INTO book_progress (book_id, reader, page, last_opened_at) VALUES (?, ?, NULL, datetime('now')) ON CONFLICT(book_id, reader) DO UPDATE SET last_opened_at = datetime('now')").run(id, reader);
      db.close();
      json(res, 200, { ok: true });
    } catch (e) { json(res, 500, { error: e.message }); }
    return true;
  ...
  if (req.method === 'POST' && (req.url === '/v1/books' || req.url.startsWith('/v1/books?'))) {
    ...
    try {
      ...
      // 书架查重（SullyOS 回填）：同名/互相包含拦下，force=1 放行
      const force = new URL(req.url, `http://localhost:${port}`).searchParams.get('force') === '1';
      if (!force) {
        const norm = String(title).replace(/\s+/g, '');
        const dupDb = getDb(true);
        const dup = dupDb.prepare('SELECT id, title FROM books').all()
          .find(b => { const t = String(b.title).replace(/\s+/g, ''); return t && norm && (t.includes(norm) || norm.includes(t)); });
        dupDb.close();
        if (dup) { json(res, 409, { error: 'duplicate', existing_id: dup.id, existing_title: dup.title, hint: `书架已有 #${dup.id}《${dup.title}》，确认非重复请加 force=1` }); return true; }
      }
  ...
  // POST /v1/books/repair-jumps — 跳转索引修复引擎未随 SullyOS 回填（EPUB 索引引擎没移植）；
  // 显式 501，让前端「修复引擎还在路上」的提示生效，而不是落到静态页返回 HTML
  if (req.method === 'POST' && req.url === '/v1/books/repair-jumps') {
    json(res, 501, { error: 'not_implemented', message: '修复引擎还在路上' });
    return true;
  }

  // /v1/reading-wishlist — 最小实现（SullyOS 回填：开书/读完时匹配愿望单），config 表存 items 数组
  if (req.url === '/v1/reading-wishlist' || req.url.startsWith('/v1/reading-wishlist?')) {
    if (roomGate(req, res)) return true;
    try {
      const db = getDb();
      const read = () => { try { return JSON.parse(db.prepare('SELECT value FROM config WHERE key = ?').get('reading_wishlist')?.value || '[]'); } catch { return []; } };
      const write = (items) => db.prepare('INSERT OR REPLACE INTO config (key, value) VALUES (?, ?)').run('reading_wishlist', JSON.stringify(items));
      if (req.method === 'GET') {
        const order = { want: 0, reading: 1, done: 2 };
        const items = read().sort((a, b) => (order[a.status] ?? 0) - (order[b.status] ?? 0) || String(b.added_at || '').localeCompare(String(a.added_at || '')));
        db.close();
        json(res, 200, { items });
        return true;
      }
      if (req.method === 'POST') {
        const body = await readBody(req);
        const { id, title, author, reason, status } = body;
        if (!title) { db.close(); json(res, 400, { error: 'title required' }); return true; }
        const items = read();
        if (id) {
          const i = items.findIndex(w => w.id === id);
          if (i >= 0) items[i] = { ...items[i], title, author: author ?? items[i].author, reason: reason ?? items[i].reason, status: status || items[i].status };
          else items.push({ id, title, author: author || null, reason: reason || null, status: status || 'want', added_at: new Date().toISOString() });
          write(items); db.close();
          json(res, 200, { ok: true, id });
        } else {
          const newId = items.reduce((m, w) => Math.max(m, Number(w.id) || 0), 0) + 1;
          items.push({ id: newId, title, author: author || null, reason: reason || null, status: status || 'want', added_at: new Date().toISOString() });
          write(items); db.close();
          json(res, 201, { ok: true, id: newId });
        }
        return true;
      }
      if (req.method === 'DELETE') {
        const id = Number(new URL(req.url, `http://localhost:${port}`).searchParams.get('id'));
        if (!id) { db.close(); json(res, 400, { error: 'id required' }); return true; }
