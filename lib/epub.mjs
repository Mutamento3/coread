...
export function parseEpub(base64Data) {
  ...
  const buf = Buffer.isBuffer(base64Data) ? base64Data : Buffer.from(base64Data, 'base64');
  ...
  while ((m = manifestRe.exec(opfXml)) !== null) {
    ...
    if (id && href && type) manifest[id] = { href, type, properties };
  }
  ...
  // EPUB3 多级兜底（SullyOS 回填）：properties="cover-image" → id 含 cover → 正文首张独立图
  if (!epubCoverFile) {
    const coverItem = Object.values(manifest).find(item => /(^|\s)cover-image(\s|$)/.test(item.properties || '') && /image/i.test(item.type));
    if (coverItem) epubCoverFile = opfDir + decodeURIComponent(coverItem.href);
  }
  if (!epubCoverFile) {
  ...
  // EPUB3 回退（SullyOS 回填）：无 NCX 时从 nav 文档的 <nav epub:type="toc"> 块提取目录
  if (tocChapters.length === 0) {
    const navEntry = Object.values(manifest).find(item => /(^|\s)nav(\s|$)/.test(item.properties || ''));
    if (navEntry) {
      const navXml = zip.readAsText(opfDir + decodeURIComponent(navEntry.href)) || '';
      const navBlock = (navXml.match(/<nav[^>]*epub:type\s*=\s*"[^"]*toc[^"]*"[^>]*>([\s\S]*?)<\/nav>/i)
        || navXml.match(/<nav[^>]*>([\s\S]*?)<\/nav>/i) || [])[1] || '';
      const aRe = /<a\s[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/gi;
      let am;
      while ((am = aRe.exec(navBlock)) !== null) {
        const t = am[2].replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' ').replace(/&amp;/g, '&').trim();
        if (t) tocChapters.push({ title: t, src: decodeURIComponent(am[1]).split('#')[0] });
      }
    }
  }
  const tocSrcSet = new Set(tocChapters.map(c => c.src));
  const tocSrcBaseSet = new Set(tocChapters.map(c => c.src.split('/').pop()));
  ...
  for (const ref of spineRefs) {
    ...
    if (tocSrcSet.has(hrefBase) || tocSrcSet.has(item.href) || tocSrcBaseSet.has(hrefBase)) {
      const ch = tocChapters.find(c => c.src === hrefBase || c.src === item.href || c.src.split('/').pop() === hrefBase);
  ...
  // 封面最后一级兜底：有的 EPUB 不写 cover 元数据，封面就是正文第一张独立图
  if (!epubCoverFile) {
    const firstImage = cleaned[0]?.match(/^\[IMG:([^\]]+)\]$/);
    if (firstImage) {
      const coverEntry = [...epubImageMap.entries()].find(([, fname]) => fname === firstImage[1]);
      if (coverEntry) epubCoverFile = coverEntry[0];
    }
  }
