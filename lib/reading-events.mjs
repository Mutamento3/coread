...
export function recordCheckpoint(db, bookId, input) {
  ...
  // start_min（放射图「几点开始读」刻度）：可选，非法就丢，不挡计时
  const startMin = Number.isInteger(input.start_min) && input.start_min >= 0 && input.start_min < 1440 ? input.start_min : null;
  return db.transaction(() => {
    ...
    db.prepare(`INSERT INTO reading_checkpoints (session_id, book_id, reading_date, elapsed_ms, start_min) VALUES (?, ?, ?, ?, ?)
      ON CONFLICT(session_id, reading_date) DO UPDATE SET elapsed_ms = MAX(elapsed_ms, excluded.elapsed_ms),
      start_min = COALESCE(excluded.start_min, reading_checkpoints.start_min)`).run(session, bookId, date, accepted, startMin);
