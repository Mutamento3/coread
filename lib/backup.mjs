...
export const SETTINGS = ['coread-human-name', 'coread-ai-name', 'coread-font-size', 'coread-brightness', 'coread-night-mode', 'coread-reading-clock', 'coread-tap-turn'];
const TABLES = {
...
export function exportBackup(settings = {}) {
  ...
  try {
    return db.transaction(() => {
      ...
      const backup = { schemaVersion: 2, createdAt: new Date().toISOString(), counts: counts(data, assets), data, assets, settings };
...
// v1→v2：book_progress 多读者化（补 reader='default' 与读完状态列），reading_checkpoints 补 start_min。
export function migrateBackup(input) {
  if (input?.schemaVersion === 1) {
    const migrated = { ...input, schemaVersion: 2, data: { ...input.data } };
    migrated.data.book_progress = (migrated.data.book_progress || []).map(row => ({
      reader: 'default',
      reading_status: row.finished_at ? 'finished' : null,
      first_finished_at: row.finished_at ?? null,
      last_finished_at: row.finished_at ?? null,
      ...row,
    }));
    migrated.data.reading_checkpoints = (migrated.data.reading_checkpoints || []).map(row => ({ start_min: null, ...row }));
    return migrated;
  }
  if (input?.schemaVersion !== 2) invalid('Unsupported schemaVersion; a version migration is required');
  return input;
}
